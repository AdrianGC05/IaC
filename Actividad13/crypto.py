# Importamos las funciones necesarias del módulo cryptography:
# - Cipher: para crear el objeto de cifrado
# - algorithms: contiene los algoritmos como AES
# - modes: contiene los modos de operación como CBC
from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes

# Importamos el backend por defecto, necesario para crear el cifrador
from cryptography.hazmat.backends import default_backend

# urandom permite generar bytes aleatorios seguros para claves e IVs
from os import urandom

def generar_clave_y_iv():
    """Genera una clave ACS de 32 bytes(AES-256) y un IV de 16 bytes."""
...
def encriptar(texto_plano, key, iv):
    ...
    # Crear objeto Cipher con:
    # - algoritmo AES usando la clave generada
    # - modo CBC usando el IV generado
    # - backend por defecto del sistema
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
    encryptor = cipher.encryptor()

    # Padding para que el texto tenga una longitud que sea multiplo del tamaño del bloque AES (16 bytes)
    # Esto es necesario para algunos modos de cifrado como CRC
    texto_plano_bytes = texto_plano.encode('utf-8')
    padding_length = 16 - (len(texto_plano_bytes) % 16)

    # Creamos el padding según la convención PKCS7: el valor del padding se repite n veces
    padding = bytes([padding_length]) * padding_length
    texto_a_cifrar = texto_plano_bytes + padding 

    # Ejecutamos la encriptación (update procesa datos, finalize cierra el proceso)
...
def desencriptar(texto_cifrado, key, iv):
    ...
    # Creamos el objeto Cipher igual que en el cifrado
    cipher = Cipher(algorithms.AES(key), modes.CBC(iv), backend=default_backend())
    decryptor = cipher.decryptor()

    # Procesamos el texto cifrado para obtener el texto desencriptado (incluye padding)
    ...
    # Convertimos los bytes a texto UTF-8
...
# 2. Texto original
texto_original = "Este es un mensaje secreto, Adrian Gaspar"
